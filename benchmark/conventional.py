"""Run a bounded, tuning-only comparison; preserves failures and untouched holdout."""
import argparse
import hashlib
import json
import platform
...
def run(out, limit):
    if limit < 1 or limit > 92:
        raise ValueError('Tuning limit must be between 1 and 92')
    ...
    identifiers = {r['id']: secrets.token_hex(16) for r in positives}
    ...
    plan = {'status': 'tuning_experiment_not_release_evaluation', 'environment': h.environment(),
            ...
            'limitations': ['Tuning results; not held-out validation', 'Unapproved rights and negative eligibility',
                           'No geometric synchronization or screenshot region search',
    ...
    for candidate in d.CONFIGS:
        ...
        for index, source in enumerate(positives + negatives):
            ...
            for transform in h.transforms() if positive else ['original']:
                ...
                start = time.perf_counter()
                try:
                    recovered = d.extract(image, candidate)
                    status, complete, error = 'ok', True, None
                except ValueError as exc:
                    recovered, status, complete, error = None, 'unsupported', False, str(exc)
                elapsed = (time.perf_counter() - start) * 1000
                predictions.append({'case_id': case_id, 'status': status, 'search_complete': complete,
                                    'detected': recovered is not None, 'decoded_ids': [recovered] if recovered else [],
